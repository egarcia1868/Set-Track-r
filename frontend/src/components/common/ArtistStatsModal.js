...
const ArtistStatsModal = ({ isOpen, onClose, concerts }) => {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        className="modal-content artist-stats-modal"
      ...
      >
              ...
              <div className="pie-chart-wrapper">
                ...
                {hoveredArtist && (
                  <div className="chart-tooltip">
                    <strong>{hoveredArtist.name}</strong>
                    <br />
                    {hoveredArtist.count} concert
                    {hoveredArtist.count !== 1 ? "s" : ""}(
                    {hoveredArtist.percentage
                      ? hoveredArtist.percentage.toFixed(1) + "%"
                      : ""}
                    )
                  </div>
                )}
                ...
                <svg width="100%" height="400" viewBox="0 0 1600 400">
                  {barData.map((bar, index) => {
                    ...
                    const barWidth = Math.max(
                      availableWidth / barData.length - 20,
                      50,
                    );
        ...
        <div className="chart-summary">
          <p>
            Total: {totalConcerts} concerts across {artistData.length} artists
          </p>
        </div>
      </div>
    </div>
